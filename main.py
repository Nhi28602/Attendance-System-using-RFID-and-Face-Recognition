...
import asyncio
import queue
from utils import findEncodings, load_training_images, send_attendance_request, send_control_device, ESP32_CAM_IP, DELTA
import datetime
...
def handle_request_with_delay(name, img, current_time):
    ...
    if name != last_name or (current_time_in_seconds - last_request_time) >= 5:
                ...
                if recognition_counter % 4 == 0:
                    ...
                    with camera_lock:
                        for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
                            ...
                            if matches[matchIndex] and (1 - minFaceDis) >= 0.7:
                            ...
                            cv2.rectangle(img, (x1, y2 - 35), (x2, y2 + 20), color, cv2.FILLED)
                            cv2.putText(img, f'id = {name}', (x1 + 6, y2 - 6), cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 255, 255), 2)                  
                            if accuracy > 0.00:
                                cv2.putText(img, f'Accuracy: {accuracy+DELTA:.2f}%', (x1 + 6, y2 + 10), cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 255, 255), 2)
